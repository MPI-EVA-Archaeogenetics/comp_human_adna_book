...
::: {.callout-note}
The practical parts @sec-install and @sec-basic of the following introduction are a condensed version of two sections of the **mobest documentation** available at **<https://nevrome.de/mobest>**. More advanced topics beyond the most basic usecase are also introduced there.
:::
...
![Distribution of the archaeogenetic samples in a genetic space constructed via multidimensional scaling.](img/mobest/overview_mds.png){width=60%}

The package then provides a framework to perform spatiotemporal interpolation using **Gaussian process regression** (GPR, "kriging") with the **laGP package** to reconstruct an ancestry field based on the ancestry measure you provided.

::: {layout-nrow=2}
![Sparse point cloud.](img/mobest/overview_3D_1.png)
...
![Similarity probability for the Stuttgart sample (@Lazaridis2014) calculated for the time slice at 5600 BC with two MDS-based ancestry components C1 and C2.](img/mobest/search_map_Stuttgart.png)
...
mobest's similarity search usually takes the perspective of an **individual sample for which we want to determine similarity probabilities** for a spatial prediction grid at a specific point in time.
...
The `locest()` arguments **`independent` and `dependent` take the spatiotemporal and genetic (as for example derived from MDS/PCA) positions of the interpolation-informing samples**.
...
Note that you can have an **arbitrary number of these components with arbitrary names**.

The **lengths** and **order** of the vectors (`samples_projected$...`) used for `create_spatpos()` and `create_obs()` all have to be identical.
...
The `locest()` argument `kernel` takes an object of the class `mobest_kernelsetting`. This type encodes **kernel configurations for each dependent variable**, so the parameters for the **Gaussian process regression (GPR)** interpolation that should be used for this variable.

These include mostly the **lengthscale parameters in space (x and y) and time**, as well as the **nugget parameter**. In very simple terms:
...
**Nugget**: Error term to model local (for observations from the same position in space and time) variability. See @Gramacy2020, specifically [here](https://bookdown.org/rbg/surrogates/chap5.html#chap5hyper).
...
Note how we scale the lengthscale parameters: `dsx` and `dsy` are set in meters (800 * 1000m = 800km) and `dt` in years (800y).

The main question naturally arising from this, is **how to set these parameters for a given dataset and research question**. There are various empirical ways to find optimal values through numerical optimization. See Supplementary Text 2 of @Schmid2023 for the approaches we applied.

::: {.callout-note}
We would argue, that the computationally expensive crossvalidation workflow to estimate the lengthscale parameters is **not always necessary** for basic applications of mobest.
...
With input data and settings out of the way we can now specify the **points in space and time** where we want to perform the search.
...
The **search time** can be specified as an integer vector of years: e.g. `search_time = c(-500, -200, 100)`.

This vector gets interpreted by `mobest::locate()` in two different ways, which can be selected with the switch argument `search_time_mode`.

- **`"relative"`**: The `search_time` is interpreted as a $\Delta t$ relative to the age of the search sample(s). Negative values point to ages that are older then the sample age, so in their relative past, and positive ones to younger ages in their relative future.
- **`"absolute"`** The values in `search_time` are interpreted as absolute ages in years BC/AD.
...
The output data type `mobest_locateoverview` has a large set of columns, **many not immediately relevant** to the basic example here. This applies especially for the variables documenting the **excessive permutation mechanics** hidden behind the relatively simple interface of `mobest::locate()`.
...
The number of rows in a `mobest_locateoverview` table can be calculated as a **product of the individual permutations**. For our example this calculation is fairly simple:
...
The number of rows in `mobest_locateproduct` will therefore be $\frac{1}{\text{Number of dependent variables}}$ times the number of rows in the input `mobest_locateoverview` table.
