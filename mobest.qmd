...
The **length** and **order** of these vectors (`samples_projected$...`) used for `create_spatpos()` and `create_obs()` all have to be identical.
...
These include mostly the **lengthscale parameters in space (x and y) and time**, as well as the **nugget parameter** (@Gramacy2020, specifically [here](https://bookdown.org/rbg/surrogates/chap5.html#chap5hyper)). In very simple terms:
...
`dsx` and `dsy` are set in meters (800 * 1000m = 800km) and `dt` in years (800y).
