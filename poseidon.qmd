...
1. **Make yourself familiar** with a number of core technologies. This is less daunting than it sounds, because: Superficial knowledge is sufficient, knowing them is useful beyond this particular task.
...
2. **Create a package** from your genotype data and fill it with a suitable set of meta and context information.
  ...
  - In a next step we modify `POSEIDON.yml`, `.janno` and `myPackage.bib` to include the context information we consider relevant. All of these files are well specified and documented, so we will only perform a minimal change for this example.
...
![Press the fork button in the top right corner to fork a repository on GitHub](img/poseidon/fork.png)
  ...
  - And then clone the fork to our computer, while omitting the large genotype data files. Note that this requires several setup steps to work correctly:
    ...
    git clone git@github.com:<yourGitHubUserName>/community-archive.git
    ```
  ...
  - With the cloned repository on our system we can copy the files into the repositories directory and commit the changes.
    ...
    cp ../myPackage myPackage
    git add myPackage
    git commit -m "added a first draft of myPackage"
    git push
    ```
  
  - In a last step we can open a Pull Request on GitHub from our fork to the original archive repository. It will be reviewed and eventually merged.
  
![When you pushed to your fork, GitHub will automatically offer to open a PR to the source repository](img/poseidon/pull_request.png)
