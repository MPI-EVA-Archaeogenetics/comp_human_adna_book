...
library(magrittr)
dat <- readr::read_tsv("fst_working/fstat_world_output.tsv") %>%
  dplyr::select(-c(c, d, Z_score_Jackknife))
datFST <- dat %>% dplyr::filter(Statistic == "FST")
datF2 <- dat %>% dplyr::filter(Statistic == "F2")
datFST
```
...
```{r}
dat %>% dplyr::arrange(-Estimate_Total)
```
...
So one of the key things to visualise is the pairwise matrix of FST: